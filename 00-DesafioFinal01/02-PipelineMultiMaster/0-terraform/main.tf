...
resource "aws_instance" "k8s_workers" {
  ...
  count         = 3
  tags = {
    Name = "vini_k8s_workers-${count.index}"
  }
...
resource "aws_security_group" "acessos_masters" {
  ...
  ingress = [
    ...
    {
      description      = "Liberando para o mundo"
      from_port        = 30002
      to_port          = 30002
      protocol         = "tcp"
      cidr_blocks      = ["0.0.0.0/0"]
...
output "security-group-workers-e-haproxy" {
  value = [ "haproxy - ${aws_security_group.acessos_haproxy.id}",
            "workers - ${aws_security_group.acessos_workers.id}",
            "master - ${aws_security_group.acessos_masters.id}"
          ]
